...
pub mod permissions {
    ...
    pub fn check_world_writable_files() -> Result<(), Box<dyn std::error::Error>> {
            ...
            .output()
        {
            ...
            if !world_writable.is_empty() {
                safe_println("    WARNING: {} world-writable files found in system directories", world_writable.len());
    ...
    pub fn check_suid_sgid_permissions() -> Result<(), Box<dyn std::error::Error>> {
        ...
        if let Ok(output) = Command::new("find")
            .args(&["/", "-type", "f", "-perm", "/6000", "-exec", "ls", "-la", "{}", ";"])
            .output()
        {
            let output_str = String::from_utf8_lossy(&output.stdout);
    ...
    pub fn check_file_ownership() -> Result<(), Box<dyn std::error::Error>> {
            ...
            .output()
        {
            ...
            if !orphan_files.is_empty() {
                safe_println("    WARNING: {} files with invalid ownership found", orphan_files.len());
    ...
    pub fn detect_permission_anomalies() -> Result<(), Box<dyn std::error::Error>> {
            ...
            .output()
        {
            ...
            if !world_rw_files.is_empty() {
                safe_println("    CRITICAL: {} files with 777 permissions in /etc", world_rw_files.len());
            ...
            .output()
        {
            ...
            if !executable_configs.is_empty() {
                safe_println("    WARNING: {} executable files in /etc directory", executable_configs.len());
            }
        }
