...
pub mod logs {
    ...
    // Safe println that handles BrokenPipe errors gracefully
    ...
    pub fn check_system_logs() -> Result<(), Box<dyn std::error::Error>> {
        ...
        if let Ok(output) = Command::new("journalctl")
            .args(&["--since", "1 hour ago", "--grep", "authentication failure", "--no-pager"])
            .output()
        {
            let output_str = String::from_utf8_lossy(&output.stdout);
            let auth_failures = output_str.lines().count();
            if auth_failures > 0 {
                safe_println(&format!("    Found {} authentication failures in last hour", auth_failures));
            }
        }
            ...
            .output()
        {
            ...
            if sudo_usage > 0 {
                safe_println(&format!("    Found {} sudo commands in last hour", sudo_usage));
            }
        }
            ...
            .output()
        {
            ...
            if security_events > 0 {
                safe_println(&format!("    Found {} security-related events in last hour", security_events));
            }
        }
        ...
        for log_file in log_files {
            if std::path::Path::new(log_file).exists() {
                if let Ok(metadata) = std::fs::metadata(log_file) {
                    ...
                    safe_println(&format!("    {}: {} bytes, modified {} seconds ago", log_file, size, modified));
                }
            }
        }
    ...
    pub fn detect_log_manipulation() -> Result<(), Box<dyn std::error::Error>> {
        ...
        if let Ok(output) = Command::new("journalctl")
            .args(&["--since", "1 day ago", "--output", "short-iso", "--no-pager", "-n", "100"])
            .output()
        {
            let output_str = String::from_utf8_lossy(&output.stdout);
            ...
            if lines.len() > 10 {
                safe_println(&format!("    Analyzed {} recent log entries for timestamp consistency", lines.len()));
