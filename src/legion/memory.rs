...
pub mod memory {
                ...
                for (i, line) in lines.iter().enumerate().take(5) {
                    if i > 0 { // Skip header
                        ...
                        if parts.len() >= 4 {
                            ...
                            safe_println(&format!("      {}% - {}", mem_usage, cmd));
                        }
                    }
                }
            }
    ...
    pub fn detect_memory_anomalies() -> Result<(), Box<dyn std::error::Error>> {
        ...
        if let Ok(output) = Command::new("journalctl")
            .args(&["--since", "1 hour ago", "--grep", "Out of memory", "--no-pager"])
            .output()
        {
            let output_str = String::from_utf8_lossy(&output.stdout);
            let oom_events = output_str.lines().count();
            if oom_events > 0 {
                safe_println(&format!("    WARNING: {} Out of Memory events detected in last hour", oom_events));
            }
        }
            ...
            .output()
        {
            ...
            for line in output_str.lines() {
                ...
                if parts.len() >= 4 {
                    if let Ok(rss_kb) = parts[2].parse::<u64>() {
                        ...
                        if rss_mb > 500 { // Processes using more than 500MB
            ...
            if !high_memory_processes.is_empty() {
                ...
                for (mem, cmd) in high_memory_processes.iter().take(5) {
                    safe_println(&format!("      {}MB - {}", mem, cmd));
                }
            }
        }
