...
/// USB device and peripheral monitoring
pub mod usb {
    use super::*;
    ...
    pub fn check_usb_devices() -> Result<(), Box<dyn std::error::Error>> {
        ...
        if let Ok(output) = Command::new("lsusb").output() {
            ...
            safe_println(&format!("    {} USB devices detected", device_count));
    ...
    pub fn detect_usb_anomalies() -> Result<(), Box<dyn std::error::Error>> {
        ...
        {
            let output_str = String::from_utf8_lossy(&output.stdout);
            let usb_access_processes: Vec<&str> = output_str.lines()
                .filter(|line| line.starts_with("p"))
                .collect();

            if !usb_access_processes.is_empty() {
                safe_println("    Processes accessing USB devices: {}", usb_access_processes.len());
            }
        }
    ...
    pub fn check_usb_history() -> Result<(), Box<dyn std::error::Error>> {
            ...
            .output()
        {
            ...
            if usb_events > 0 {
                safe_println("    {} USB-related events in system logs (last 24h)", usb_events);
            }
        }
