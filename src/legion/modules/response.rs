...
impl ResponseEngine {
    ...
    pub async fn execute_response(
        &self,
        anomaly: &Anomaly,
    ) -> Result<Vec<ResponseAction>, ResponseError> {
        ...
        // Log incident
        self.log_incident(anomaly, &executed_actions, true, None)
            .await;
    ...
    async fn execute_action(&self, action: &ResponseAction) -> Result<(), ResponseError> {
        ...
        match action {
            ResponseAction::IsolateProcess { pid } => self.isolate_process(*pid).await,
            ResponseAction::BlockNetwork { ip, port } => self.block_network(ip, *port).await,
            ResponseAction::QuarantineFile { path } => self.quarantine_file(path).await,
            ResponseAction::KillProcess { pid } => self.kill_process(*pid).await,
            ResponseAction::DisableService { name } => self.disable_service(name).await,
    ...
    async fn isolate_process(&self, pid: u32) -> Result<(), ResponseError> {
        ...
        if output.status.success() {
            ...
            Err(ResponseError::ExecutionFailed(
                "Process isolation failed".to_string(),
            ))
        }
    }
    ...
    async fn block_network(&self, ip: &str, port: u16) -> Result<(), ResponseError> {
        ...
        if output.status.success() {
            ...
            Err(ResponseError::ExecutionFailed(
                "Network blocking failed".to_string(),
            ))
        }
    }
    ...
    async fn quarantine_file(&self, path: &str) -> Result<(), ResponseError> {
        ...
        std::fs::create_dir_all(&self.quarantine_dir).map_err(|e| {
            ResponseError::ExecutionFailed(format!("Failed to create quarantine dir: {}", e))
        })?;

        // Move file to quarantine
        std::fs::rename(&source_path, &quarantine_path).map_err(|e| {
            ResponseError::ExecutionFailed(format!("Failed to quarantine file: {}", e))
        })?;
    ...
    async fn execute_custom_command(
        ...
        if output.status.success() {
            ...
            Err(ResponseError::ExecutionFailed(format!(
                "Command failed: {}",
                stderr
            )))
        }
