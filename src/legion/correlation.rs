...
impl IncidentCorrelator {
    ...
    fn evaluate_rule(&self, rule: &CorrelationRule, trigger_event: &SecurityEvent) -> Option<Incident> {
        // Get events in the time window
        let window_start = trigger_event.timestamp - Duration::minutes(rule.correlation_logic.time_window_minutes);
        let relevant_events: Vec<&SecurityEvent> = self.event_buffer
            .iter()
        ...
        match rule.correlation_logic.logic_type {
            LogicType::Any => {
                self.evaluate_any_logic(rule, &relevant_events, trigger_event)
            }
            LogicType::All => {
                self.evaluate_all_logic(rule, &relevant_events, trigger_event)
            }
            LogicType::Weighted => {
    ...
    fn evaluate_any_logic(&self, rule: &CorrelationRule, events: &[&SecurityEvent], trigger_event: &SecurityEvent) -> Option<Incident> {
        // Check if any condition matches any event
        for condition in &rule.conditions {
            for event in events {
                if self.condition_matches(condition, event) {
                    return Some(self.create_incident_from_rule(rule, vec![trigger_event.clone()], 0.8));
                }
            }
        }
        None
    ...
    fn evaluate_all_logic(&self, rule: &CorrelationRule, events: &[&SecurityEvent], _trigger_event: &SecurityEvent) -> Option<Incident> {
    ...
    fn evaluate_weighted_logic(&self, rule: &CorrelationRule, events: &[&SecurityEvent], _trigger_event: &SecurityEvent) -> Option<Incident> {
        ...
        if total_weight >= rule.correlation_logic.threshold {
            Some(self.create_incident_from_rule(rule, matched_events,
    ...
    fn evaluate_sequence_logic(&self, rule: &CorrelationRule, events: &[&SecurityEvent], _trigger_event: &SecurityEvent) -> Option<Incident> {
    ...
    fn condition_matches(&self, condition: &CorrelationCondition, event: &SecurityEvent) -> bool {
        ...
        match condition.operator {
            ...
            ConditionOperator::GreaterThan => {
                field_value.parse::<f64>().unwrap_or(0.0) > condition.value.parse::<f64>().unwrap_or(0.0)
            }
    ...
    fn create_incident_from_rule(&self, rule: &CorrelationRule, events: Vec<SecurityEvent>, confidence: f64) -> Incident {
    ...
    pub fn add_correlation_rule(&mut self, rule: CorrelationRule) {
        ...
        self.correlation_rules.sort_by(|a, b| b.priority.cmp(&a.priority));
    }
    ...
    pub fn cleanup_old_incidents(&mut self, max_age_hours: i64) {
        ...
        self.active_incidents.retain(|_, incident| incident.created_at > cutoff_time);
    }
...
impl CorrelationManager {
    pub fn new(window_duration_minutes: i64, max_events: usize) -> Self {
        Self {
            correlator: Arc::new(RwLock::new(IncidentCorrelator::new(window_duration_minutes, max_events))),
        }
    }
    ...
    pub async fn get_active_incidents(&self) -> Vec<Incident> {
        ...
        correlator.get_active_incidents().into_iter().cloned().collect()
    }
