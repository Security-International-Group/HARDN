...
impl ProcessBehavior {
    ...
    fn is_suspicious_connection(&self, conn: &Connection) -> bool {
        ...
        if let Ok(port) = conn.remote_addr.split(':').last().unwrap_or("").parse::<u16>() {
        ...
        if let Ok(ip) = conn.remote_addr.split(':').next().unwrap_or("").parse::<IpAddr>() {
            match ip {
                IpAddr::V4(ipv4) => {
                ...
                IpAddr::V6(_) => {
                    ...
                    if conn.remote_addr.starts_with("fc00:") || conn.remote_addr.starts_with("fd00:") {
                        if !self.is_expected_private_connection(&self.name) {
                            return true;
                        }
                    }
    ...
    fn has_unusual_connection_pattern(&self) -> bool {
        ...
        let recent_connections: Vec<_> = self.network_connections.iter()
            .filter(|conn| {
                let duration = Utc::now().signed_duration_since(conn.timestamp.unwrap_or(Utc::now()));
    ...
    fn is_expected_private_connection(&self, process_name: &str) -> bool {
        let expected_processes = ["sshd", "apache2", "nginx", "mysql", "postgresql", "redis", "mongodb"];
...
impl BehavioralAnalyzer {
    ...
    pub fn analyze_process(&mut self, pid: u32, mut behavior: ProcessBehavior) -> BehaviorClassification {
        behavior.update_behavior_score();
        let classification = behavior.analyze_behavior();

        if classification == BehaviorClassification::Suspicious || classification == BehaviorClassification::Malicious {
